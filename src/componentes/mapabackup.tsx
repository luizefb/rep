...
const Mapa2 = () => {
  ...
  useEffect(() => {
    const verificarMarkers = setTimeout(() => {
      if (!coordenadas) {
        console.log("Marcadores não renderizaram, forçando atualização...");
        setForceRender((prev) => prev + 1);
      }
    }, 3000);

    return () => clearTimeout(verificarMarkers);
  }, [coordenadas]);
  ...
  return (
    <View style={styles.container}>
      <MapView key={forceRender} style={styles.map} mapStyle={styleUrl}>
        <Camera
          centerCoordinate={
